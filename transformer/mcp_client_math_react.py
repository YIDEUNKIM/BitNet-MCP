...
# The prompt is kept the same to maintain the original intent of generating only the expression.
    ...
    async def _get_available_tools(self) -> List[Dict[str, Any]]:
        ...
        try:
            async with Client(self.config.server_url) as client:
                ...
                # MCP 서버에 'calculate' 도구가 등록되어 있다고 가정합니다.
                # 이는 doc의 내용에 기반한 추가 도구입니다.
                calculate_tool_schema = {
                    "name": "calculate",
                    "description": "Calculates the result of a mathematical expression. The expression must be a valid Python string.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "expression": {
                ...
                # 캐시에 'calculate' 도구가 없으면 추가합니다.
                if not any(t['name'] == 'calculate' for t in self._tools_cache):
                    self._tools_cache.append(calculate_tool_schema)
...
"""
    ...
    # ★★★★★ 수정된 부분 ★★★★★
    async def _solve_math_with_self_correction(self, user_query: str) -> Optional[float]:
        # Step 1: Generate initial Python expression
        expression = None
        retry_count = 0
        max_retries = 3
        while retry_count < max_retries:
            prompt_context = ""
            if retry_count > 0:
                print(f" - Retrying math problem (attempt {retry_count + 1}/{max_retries})...")
                prompt_context = f"\n[Previous Attempt]\nExpression: {expression}\nError: {last_error}\n[Instruction] Please correct the expression based on the error. Just output the corrected expression."

            prompt = QUERY_TO_EXPRESSION_PROMPT.format(query=user_query) + prompt_context
            expression = self.slm.query(prompt, max_new_tokens=256, temperature=0.1).strip()
            print(f" - Generated expression: {expression}")

            # Defensive check for LLM's response
            if not expression or not re.match(r'^[\s\(\)0-9\.\+\-\*\/\%]+$', expression):
                last_error = "Invalid or empty expression from LLM. It must be a valid Python expression."
                print(f" - {last_error}: '{expression}'")
                retry_count += 1
                continue
            ...
            if not calc_result_dict.get("success"):
                last_error = calc_result_dict.get('error', 'Unknown error')
                print(f" - Calculation failed: {last_error}")
                retry_count += 1
                continue
            ...
            try:
                # Check for dictionary format first
                ...
                if isinstance(parsed_output, dict) and 'result' in parsed_output:
                    ...
                    # Assume it's a simple number string
                ...
                last_error = f"Invalid MCP result format: {result_value_str} (Error: {e})"
                print(f" - {last_error}")
                retry_count += 1
                continue

        print(f" - Failed to solve math problem after {max_retries} retries.")
        return None
    ...
    async def route_query(self, user_query: str):
        ...
        if not available_tools:
            print("No tools available. Handling as general chat.")
            ...
            if selected_tool_name == "calculate":
                print(f"Selected tool '{selected_tool_name}' for math problem.")
                result_value = await self._solve_math_with_self_correction(user_query)
                if result_value is not None:
                    response = self._generate_natural_response(user_query, str(result_value), "calculate")
                else:
                    response = self._handle_general_chat(user_query)
            elif selected_tool_name != "chat":
    ...
    async def test_gsm8k(self, max_samples: int = 100, csv_filename: str = "gsm8k_results.csv"):
        ...
        for i, sample in enumerate(test_set):
            ...
            # ★★★★★ 수정된 부분: solve_math 대신 _solve_math_with_self_correction 호출
            predicted = await self._solve_math_with_self_correction(question)
            # ========================================
